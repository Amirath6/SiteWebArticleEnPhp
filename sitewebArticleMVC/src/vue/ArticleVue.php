...
class ArticleVue {
    ...
    public function makeArticlePage($id, Article $article){
        ...
        $this->title = "<p style=\"color:red\"><< {$titleArt} >>,</p> créé le " . $dateArt->format("Y-m-d") . " par " .$auteurArt . "<br><br>";

        $this->content .= "<br><h2><u style=\"color:blue; margin-left:600px;\">Description de l'article</u></h2>\n";
        $this->content .= "<br><p style=\"margin-left:100px; background-color: linear-gradient(0.25turn, #3f87a6, #ebf8e1, #f69d3c); border-radius:12em;\">$contenuArt</p>\n";

        $this->content .= "<div>\n";
        $this->content .= "<p class=\"button1\"><a href=\"".$this->router->getArticleModifPageURL($id)."\">Modifier</a></p>\n";
        $this->content .= "<p class=\"button1\"><a href=\"".$this->router->getArticleDeletionURL($id)."\">Supprimer</a></p>\n";
        $this->content .= "</div>\n";
    }
